...
const SessionManager = {
    getPartner: (userID) => {
        let queryStatement = `SELECT user1, user2 FROM user_groups WHERE (user1=${userID} OR user2=${userID}) AND status="valid"`;
    ...
    _testInsertMockAnswers: ((user1, ans1, user2, ans2, courseID, problemID) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            let query1 = "INSERT INTO courseware_studentmodule ";
            query1    += "(module_type, module_id, course_id, state, done, created, modified, student_id) ";
            query1    += `values("problem", "${problemID}", "${courseID}", "${stateString1}", "na", Now(), Now(), ${user1}) `;
            query1    += `ON DUPLICATE KEY UPDATE state="${stateString1}", modified=Now();`;

            dbhandler.connect(DATABASES.EDX)
                .then(connection => dbhandler.queryNoRelease(connection, query1))
                .then((connection, results, fields) => {
                    let query2 = "INSERT INTO courseware_studentmodule ";
                    query2    += "(module_type, module_id, course_id, state, done, created, modified, student_id) ";
                    query2    += `values("problem", "${problemID}", "${courseID}", "${stateString2}", "na", Now(), Now(), ${user2}) `;
                    query2    += `ON DUPLICATE KEY UPDATE state="${stateString2}", modified=Now();`;

                    return dbhandler.query(connection, query2);
                })
                .then((results, fields) => {
                    resolve();
                })
