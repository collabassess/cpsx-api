const mysql          = require('mysql'),
      ...
      // This file MUST be handwritten. An example db_credentials.js file is provided.
...
const DatabaseHandler = {
    ...
    cluster: () => {
        if (DatabaseHandler._poolCluster === undefined) {
            debug("Making pool cluster...");

            // The "multipleStatements" flag is rather self-evident but necessary for chaining queries
            let cluster = mysql.createPoolCluster({multipleStatements: true});

            DB_CONFIGS.forEach(dbconf => {
                cluster.add(dbconf.id, dbconf.config);
            });

            DatabaseHandler._poolCluster = cluster;
        }
